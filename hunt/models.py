...
class GameUser(models.Model):
    ...
    def ranking(self):
        ...
        rank_three = 0
        if self.level == 1:
            rank_three = GameUser.objects.filter(user__is_staff=False,level=self.level,score=self.score,timestamp__lt=self.timestamp-timedelta(minutes-20)).count()
        else:
            rank_three = GameUser.objects.filter(user__is_staff=False,level=self.level,score=self.score,last_attempt__lt=self.last_attempt-timedelta(minutes-20)).count()
